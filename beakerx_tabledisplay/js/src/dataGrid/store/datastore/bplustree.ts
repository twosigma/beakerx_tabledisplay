...
export class BPlusTree<T> {
   ...
   * `O(log32 n)`
   */
  slice(start?: number, stop?: number): Private.ForwardIterator<T> {
   ...
   * `O(log32 n)`
   */
  retroSlice(start?: number, stop?: number): Private.RetroIterator<T> {
   ...
   * `O(n log32 n)`
   */
  assign(items: IterableIterator<T>): void {
    this.clear();
    this.update(items);
  }
   ...
   * `O(k log32 n)`
   */
  update(items: IterableIterator<T>): void {
    each(items, item => {
      this.insert(item);
    });
  }
...
namespace Private {
   ...
   * `O(log32 n)`
   */
  export function iterItems<T>(node: Node<T>): ForwardIterator<T> {
   ...
   * `O(log32 n)`
   */
  export function retroItems<T>(node: Node<T>): RetroIterator<T> {
  ...
  export function sliceItems<T>(
    ...
    if (count === 0) {
      return new ForwardIterator<T>(null, 0, 0);
    }
  ...
  export function retroSliceItems<T>(
    ...
    // Bail early if there is nothing to iterate.
    if (count === 0) {
      return new RetroIterator<T>(null, 0, 0);
    }
  ...
  export class ForwardIterator<T> {
    ...
    next(): IteratorResult<T | undefined> {
      ...
      return {
        ...
        value: this._node.items[this._index++]
      };
    }
